...
for(cutoff_month in seq(outsample_start+1, outsample_end, update_interval)) {
  ...
  if(cutoff_month == outsample_start+1) {
    fullModels <- GetFullModels(BernoulliInSample, MonthlyInSample, ifelse(bigdata, bigglm, glm))
...
pdf(paste('roc_bernoulli_', bigdataFileIndicator, industryTag, '.pdf', sep = ''))
...
pdf(paste('roc_', bigdataFileIndicator, industryTag, '.pdf', sep = ''))
